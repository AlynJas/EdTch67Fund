...
import { Wallet, Users, LayoutDashboard, LogOut, LogIn, PlusCircle, CheckCircle2, Clock, ShieldAlert, Edit, Search, Calendar, X, Bell, MessageSquare, Check, History, Target, PieChart, ArrowUpCircle, ArrowDownCircle, FileText, Image as ImageIcon, BookOpen, QrCode, Loader2, Upload } from 'lucide-react';

// นำเข้า Supabase Client
import { supabase } from './supabaseClient'; 

// --- ข้อมูลจำลองส่วนที่ยังไม่ได้เอาขึ้น Database ---
const mockStudents = [
...
const TERMS = [
  '1/1', '1/2', '2/1', '2/2', '2/b2', '3/1', '3/b1', '3/2', '3/b2', '4/1', '4/b1', '4/2', '4/b2'
];
...
export default function App() {
  ...
  // State ของ Transactions ให้เริ่มจากอาร์เรย์ว่าง
  ...
  // ---------------------------------------------------------------------------
  // SUPABASE: ดึงข้อมูล (Fetch) และแบบ Real-time
  // ---------------------------------------------------------------------------
  useEffect(() => {
    ...
    const fetchTransactions = async () => {
      const { data, error } = await supabase
        .from('transactions')
        .select('*')
        .order('timestamp', { ascending: false }); // เรียงล่าสุดไว้บนสุด
    ...
    // 2. สมัครรับข้อมูล Real-time (ใครอัปเดตปุ๊บ หน้าจอเปลี่ยนปั๊บ)
    const subscription = supabase
      .channel('public:transactions')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'transactions' }, payload => {
        // ดึงข้อมูลใหม่ทั้งหมดทุกครั้งที่มีการเปลี่ยนแปลงเพื่อให้เรียงลำดับถูกต้องเสมอ
        fetchTransactions();
      })
      .subscribe();
  ...
  const openRecordModal = (student) => {
    setRecordTarget(student); setAmount(''); setPaymentStep('input'); setRecordModalOpen(true);
  };
  ...
  // ---------------------------------------------------------------------------
  // SUPABASE: เพิ่มรายการจ่ายเงินของนศ. (Insert)
  // ---------------------------------------------------------------------------
  const simulatePaymentReceived = async () => {
    ...
    const newTransaction = {
      ...
      studentId: recordTarget.id, 
      studentName: recordTarget.name,
      fundType: activeTab, 
      term: selectedTerm, 
      amount: parsedAmount, 
  ...
  // ---------------------------------------------------------------------------
  // SUPABASE: อัปโหลดสลิปเข้า Storage และอัปเดต URL (Update)
  // ---------------------------------------------------------------------------
  const handleUploadSlip = async (e, txId) => {
    ...
    try {
      setSuccessMsg('กำลังอัปโหลดสลิป...');
      const fileExt = file.name.split('.').pop();
      const fileName = `${txId}_${Date.now()}.${fileExt}`;
      const filePath = `${fileName}`;

      // 1. อัปโหลดรูปขึ้น Storage Bucket ชื่อ 'slips'
      ...
      // 2. ดึง Public URL ของรูปนั้น
      const { data: { publicUrl } } = supabase.storage
        .from('slips')
        .getPublicUrl(filePath);

      // 3. เอา URL ไปอัปเดตในตาราง transactions
      const { error: updateError } = await supabase
        .from('transactions')
        .update({ slipUrl: publicUrl })
        .eq('id', txId);
  ...
  // ---------------------------------------------------------------------------
  // SUPABASE: เพิ่มรายการอื่นๆ (Insert)
  // ---------------------------------------------------------------------------
  const handleRecordOther = async (e) => {
    e.preventDefault();
    if (!currentUser || !otherAmount || !otherDescription) return;
    
    setSuccessMsg('กำลังบันทึกข้อมูล...');
    let publicUrl = null;

    // ถ้ามีไฟล์สลิปให้อัปโหลดก่อน
    if (otherSlip) {
      const fileExt = otherSlip.name.split('.').pop();
      const fileName = `other_${Date.now()}.${fileExt}`;
      const { error: uploadError } = await supabase.storage.from('slips').upload(fileName, otherSlip);
      if (!uploadError) {
        publicUrl = supabase.storage.from('slips').getPublicUrl(fileName).data.publicUrl;
      }
    }
  ...
  // ---------------------------------------------------------------------------
  // SUPABASE: การแก้ไขรายการ (Update & JSONB History)
  // ---------------------------------------------------------------------------
  const submitEdit = async (e) => {
    ...
    const newDesc = editingTx.type !== 'student_payment' ? editDescription : editingTx.description;
    
    // เตรียมข้อมูลประวัติการแก้ไข
    const historyEntry = { 
      action: 'edit', 
      amount: newAmount, 
      description: editingTx.type !== 'student_payment' ? editDescription : undefined, 
      recordedBy: currentUser.name, 
      timestamp: newTimestamp 
    };
    
    const updatedHistory = [...(editingTx.history || []), historyEntry];

    // อัปเดตข้อมูลบน Database
    const { error } = await supabase
      ...
      .eq('id', editingTx.id); // หาจาก ID (UUID)
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-800 font-sans pb-12">
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        ...
        {/* --- MODALS --- (โค้ด Modal ทั้งหมดยังเหมือนเดิม) */}
